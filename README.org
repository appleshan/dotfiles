...
这是一位 Arch Linux 折腾者的配置沉淀。目标是『可重装性优先』——任意机器一条命令即可还原整套环境；
并非常见的『装好即用』型方案，欢迎拷贝，但请接受『配置需要持续打理』的折腾成本。

- *折腾美学优先* —— 配置即文档，README 与 dot 文件同仓演进，不留第二份信源
- *AI 协作同仓* —— ~claude-code~ / ~codex~ / ~gemini-cli~ 的协作指令与安全钩子作为 dotfiles 一部分纳入版本控制
- *工作流信仰* —— 偏好 Topic-branch、不可变软链接，配置通过 =stow= 而非拷贝，每次改动在 Git 中皆有迹可循
